...
--
-- IMPORTANT:
-- Create the database once, then connect to it before running this
-- script. The commands are shown below for reference:
--
-- CREATE DATABASE OnlineBookstore;
-- \c OnlineBookstore
--
-- If the database already exists, do not run CREATE DATABASE again.
-- Run this script from the repository root directory so the relative
...
-- 1. DATABASE SETUP
-- =============================================================
-- Connect to the existing OnlineBookstore database in psql:
--
-- \c OnlineBookstore
--
-- The table definitions and analysis below should be run after
...
-- 3. DATA IMPORT
-- =============================================================
-- These are PostgreSQL psql \copy commands.
...
-- Keep SERIAL sequences aligned with the imported IDs.
SELECT setval(
    pg_get_serial_sequence('books', 'book_id'),
    COALESCE((SELECT MAX(Book_ID) FROM Books), 1),
    true
);
